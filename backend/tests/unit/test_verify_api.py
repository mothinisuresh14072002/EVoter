...
def test_verify_rejects_swapped_session_types_and_cleans_up():
    reference_session_id = create_session({"image": "reference", "type": "live"}, ttl_seconds=60)
    live_session_id = create_session({"image": "live", "type": "aadhaar"}, ttl_seconds=60)
    response = asyncio.run(verify(VerifyRequest(reference_session_id=reference_session_id, live_session_id=live_session_id)))
